...
## Quando Usar
- Quando precisar de modelagem de dados
...
ctx = Context()
ctx.set("input", "value")
result = orch.run("14_database", ctx)
```
...
results = orch.run_pipeline(["14_database", "security"], ctx)
for r in results:
...
## Referências
- Ver `framework/Engineering/14_database.py` para implementação
