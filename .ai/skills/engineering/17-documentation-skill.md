...
## Quando Usar
- Quando precisar de documentação técnica
...
ctx = Context()
ctx.set("input", "value")
result = orch.run("17_documentation", ctx)
```
...
results = orch.run_pipeline(["17_documentation", "security"], ctx)
for r in results:
...
## Referências
- Ver `framework/Engineering/17_documentation.py` para implementação
